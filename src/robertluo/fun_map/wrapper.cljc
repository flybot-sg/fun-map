...
(defn- raw-keys
  "Get raw keys from a map without triggering unwrapping.
   On host platforms, uses rawSeq to avoid triggering unwrapping.
...
(defn- wrap-fn-error
  ...
  (try
    ...
    (catch #?(:clj NullPointerException
              :cljr System.NullReferenceException
              :cljs js/Error) e
      (throw (ex-info (str "Error computing key " (pr-str k)
                           (when (seq *unwrap-stack*)
                             (str " (dependency chain: "
                                  ...
                                  ")"))
                           ": " #?(:clj (.getMessage e) :cljr (.Message e) :cljs (.-message e))
...
#?(:clj
   ...
   :cljr
   (do
     (defmethod print-method FunctionWrapper [^FunctionWrapper o ^System.IO.TextWriter wtr]
       (.Write wtr (str "<<" (.-f o) ">>")))

     (defmethod print-method CachedWrapper [^CachedWrapper o ^System.IO.TextWriter wtr]
       (.Write wtr
               (str "<<"
                    (let [v (-> (.-a-val-pair o) deref first)]
                      (if (= ::unrealized v) "unrealized" v))
                    ">>")))

     (defmethod print-method TracedWrapper [^TracedWrapper o ^System.IO.TextWriter wtr]
       (.Write wtr (str "<<traced:" (.-wrapped o) ">>")))))
